...
export function SessionView({ session, onEndSession }: SessionViewProps) {
  ...
  const processUserMessage = async (userText: string) => {
    ...
    try {
      ...
      const response = await aiServiceRef.current!.sendMessage(userText, token!);
  ...
  const handleEndSession = async () => {
    ...
    if ((!extractedTasks || extractedTasks.length === 0) && !extractedAnalysis && messages.length > 0) {
      try {
        ...
        const response = await aiServiceRef.current!.sendMessage(endingPrompt, token!);
      ...
      <div className="relative z-10 p-6 space-y-3">
        ...
        {isListening ? (
          ...
          <button
            ...
            className={`w-full py-5 rounded-2xl flex items-center justify-center gap-3 font-semibold text-lg transition-all transform ${isSpeaking || isProcessing
