...
export class AIService {
  ...
  constructor(language: 'es' | 'en' = 'es', userName?: string, assistantName?: string) {
    ...
    this.config = {
      provider: 'groq',
      apiKey: 'backend-proxy' // Placeholder, actual key is in backend
    };
  ...
  private async callGroq(): Promise<string> {
    // Use backend proxy endpoint instead of calling Groq directly
    ...
    if (!response.ok) {
      const error = await response.json();
      throw new Error(error.error || 'Error en AI API');
    }
