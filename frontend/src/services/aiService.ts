...
export class AIService {
  ...
  constructor(language: 'es' | 'en' = 'es', userName?: string, assistantName?: string) {
    ...
    // Config is now just for holding state if needed, but API key is on backend
    this.config = {
      provider: 'groq',
      apiKey: 'backend-proxy' // Placeholder
    };
  ...
  isConfigured(): boolean {
    return true; // Always configured as we use backend
  }

  async sendMessage(userMessage: string, token: string): Promise<string> {
    ...
    try {
      ...
      if (!response.ok) {
        throw new Error('Error calling AI Backend');
      }
  ...
  // Removed private call methods as they are now in backend
}
